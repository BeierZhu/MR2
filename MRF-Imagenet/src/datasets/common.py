...
def get_features(is_train, image_encoder, dataset, device):
    ...
    if image_encoder.cache_dir is not None and len(cached_files) > 0:
        ...
        for cached_file in cached_files:
            ...
            print("Trying to load cached feature file from:", cached_file)
            data[name] = torch.load(cached_file, map_location=device)
    else:
        ...
        if image_encoder.cache_dir is None:
            ...
            for name, val in data.items():
                ...
                torch.save(val, f'{cache_dir}/{name}.pt', _use_new_zipfile_serialization=False)
